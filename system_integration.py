"""System integration module for CompleteIQ."""
...
class CompetitiveIntelligenceSystem:
    """Main system orchestration and integration."""
    ...
    def __init__(self, config: SystemConfig):
        ...
        # Initialize Langfuse if tracing enabled
    ...
    def initialize(self) -> tuple[bool, List[str]]:
        ...
        try:
            ...
            # Process catalogs with tracing
            print("[INFO] Processing catalogs...")
            company_x_products = self.processor.process_catalog_with_tracing(
                self.company_x_catalog
            )
            company_y_products = self.processor.process_catalog_with_tracing(
                self.company_y_catalog
            )
            ...
            self.beacon = Beacon(self.search_engine, self.langfuse, self.session_id)
            self.nexus = Nexus(self.search_engine, self.langfuse, self.session_id)
            self.verse = Verse(self.search_engine, self.langfuse, self.session_id)
    ...
    async def analyze_competitors(self, products: Optional[List[Dict]] = None) -> CompetitiveReport:
        """Run competitive analysis across all agents.

        Args:
            products: Optional list of products to analyze. Uses loaded products if None.

        Returns:
            CompetitiveReport with aggregated insights
        """
        ...
        try:
            ...
            # Run orchestrator for parallel agent execution
            print("[INFO] Running agents in parallel...")
            await self.orchestrator.run_async(products)
            
            # Aggregate results from orchestrator
            ...
            overall_recommendation = self._generate_recommendation(
                price_analysis, feature_analysis, marketing_insights
            )
            ...
            return CompetitiveReport(
                ...
                feature_analysis={},
                marketing_insights={},
                overall_recommendation="Analysis failed",
    ...
    def health_check(self) -> HealthCheckResult:
        ...
        if components["search_engine"]:
            try:
                ...
                details["search_engine_products"] = stats.get("product_count", 0)
            except Exception as e:
        ...
        # Overall health
        overall_healthy = (
            all(components.values()) and len(errors) == 0 and len(self.normalized_products) > 0
        )
    ...
    def _generate_recommendation(
        self, price_analysis: Dict, feature_analysis: Dict, marketing_insights: Dict
    ) -> str:
        """Generate overall recommendation from agent outputs.

        Args:
            price_analysis: Beacon output
            feature_analysis: Nexus output
            marketing_insights: Verse output
        ...
        try:
            ...
            # Build recommendation from agent insights
            parts = []
    ...
    def _calculate_confidence(
        ...
        """Calculate overall confidence score from agent outputs.
        ...
        # Base confidence on agent success status
