...
_COLORS = {NODE_PRODUCT: '#4FC3F7', NODE_FEATURE: '#AED581', NODE_COMPANY: '#FFB74D', NODE_CATEGORY: '#CE93D8'}
_SHAPES = {NODE_PRODUCT: 'dot', NODE_FEATURE: 'diamond', NODE_COMPANY: 'star', NODE_CATEGORY: 'triangle'}
...
class ProductKnowledgeGraph:
    ...
    def add_products(self, products: List[Dict[str, Any]]) -> None:
        ...
        for p in products:
            ...
            self.graph.add_node(pid, node_type=NODE_PRODUCT, label=p.get('product_name', pid), company=company, category=category, price=p.get('effective_price', p.get('price', 0)), features=features)
            cid = f'company:{company}'
            if cid not in self.graph:
                self.graph.add_node(cid, node_type=NODE_COMPANY, label=company)
            ...
            if cat_id not in self.graph:
                self.graph.add_node(cat_id, node_type=NODE_CATEGORY, label=category)
            self._categories.add(category)
            self.graph.add_edge(pid, cat_id, edge_type=EDGE_IN_CATEGORY, weight=1.0)
            for feat in features:
                fid = f'feature:{feat}'
                if fid not in self.graph:
                    self.graph.add_node(fid, node_type=NODE_FEATURE, label=feat)
    ...
    def _build_competition_edges(self) -> None:
        product_nodes = [(nid, d) for nid, d in self.graph.nodes(data=True) if d.get('node_type') == NODE_PRODUCT]
        for i, (pid_a, data_a) in enumerate(product_nodes):
            for pid_b, data_b in product_nodes[i + 1:]:
                if data_a['category'] == data_b['category'] and data_a['company'] != data_b['company']:
                    ...
                    self.graph.add_edge(pid_a, pid_b, edge_type=EDGE_COMPETES_WITH, weight=round(jaccard, 3), category=data_a['category'])
    ...
    def find_competitors(self, product_id: str) -> List[Dict[str, Any]]:
        ...
        for neighbour in self.graph.neighbors(product_id):
            ...
            if edge.get('edge_type') == EDGE_COMPETES_WITH:
                ...
                competitors.append({'product_id': neighbour, 'product_name': node.get('label', ''), 'company': node.get('company', ''), 'category': node.get('category', ''), 'price': node.get('price', 0), 'similarity': edge.get('weight', 0)})
    ...
    def find_feature_gaps(self, company: str) -> List[Dict[str, Any]]:
        ...
        for feat in sorted(missing):
            owners = [comp for comp, fs in competitor_features.items() if feat in fs]
            gaps.append({'feature': feat, 'available_from': owners, 'competitor_count': len(owners)})
        trace.output = {'gap_count': len(gaps)}
        return gaps

    def find_similar_products(self, product_id: str, min_similarity: float=0.0) -> List[Dict[str, Any]]:
        ...
        for nid, data in self.graph.nodes(data=True):
            ...
            if jaccard >= min_similarity:
                results.append({'product_id': nid, 'product_name': data.get('label', ''), 'company': data.get('company', ''), 'category': data.get('category', ''), 'price': data.get('price', 0), 'shared_features': sorted(target_feats & other_feats), 'similarity': round(jaccard, 3)})
        ...
        for neighbour in self.graph.neighbors(fid):
            ...
            if edge.get('edge_type') == EDGE_HAS_FEATURE:
                ...
                if node.get('node_type') == NODE_PRODUCT:
                    products.append({'product_id': neighbour, 'product_name': node.get('label', ''), 'company': node.get('company', ''), 'category': node.get('category', ''), 'price': node.get('price', 0)})
        return products
    ...
    def get_company_products(self, company: str) -> List[Dict[str, Any]]:
        ...
        for neighbour in self.graph.neighbors(cid):
            ...
            if edge.get('edge_type') == EDGE_MANUFACTURED_BY:
                node = self.graph.nodes[neighbour]
                if node.get('node_type') == NODE_PRODUCT:
                    products.append({'product_id': neighbour, 'product_name': node.get('label', ''), 'category': node.get('category', ''), 'price': node.get('price', 0)})
        return products
    ...
    def get_category_stats(self) -> List[Dict[str, Any]]:
        ...
        for cat, info in sorted(cat_stats.items()):
            prices = info['prices']
            results.append({'category': cat, 'product_count': info['count'], 'avg_price': round(sum(prices) / len(prices), 2) if prices else 0, 'min_price': min(prices) if prices else 0, 'max_price': max(prices) if prices else 0})
        return results
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        return {'total_nodes': self.graph.number_of_nodes(), 'total_edges': self.graph.number_of_edges(), 'products': type_counts.get(NODE_PRODUCT, 0), 'features': type_counts.get(NODE_FEATURE, 0), 'companies': type_counts.get(NODE_COMPANY, 0), 'categories': type_counts.get(NODE_CATEGORY, 0), 'has_feature_edges': edge_counts.get(EDGE_HAS_FEATURE, 0), 'competes_with_edges': edge_counts.get(EDGE_COMPETES_WITH, 0), 'manufactured_by_edges': edge_counts.get(EDGE_MANUFACTURED_BY, 0), 'in_category_edges': edge_counts.get(EDGE_IN_CATEGORY, 0)}

    def visualize(self, output_path: str='./outputs/knowledge_graph.html', height: str='800px', width: str='100%', show_features: bool=True) -> str:
        trace = _trace('kg_visualize')
        trace.input = {'output_path': output_path, 'show_features': show_features}
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        net = Network(height=height, width=width, bgcolor='#1a1a2e', font_color='white', directed=False, notebook=False)
        ...
        for nid, data in self.graph.nodes(data=True):
            ...
            net.add_node(nid, label=label, color=color, shape=shape, size=size, title='<br>'.join(title_parts))
        for src, dst, data in self.graph.edges(data=True):
            etype = data.get('edge_type', '')
            if not show_features and etype == EDGE_HAS_FEATURE:
                continue
            edge_colors = {EDGE_HAS_FEATURE: '#66BB6A', EDGE_COMPETES_WITH: '#EF5350', EDGE_MANUFACTURED_BY: '#FFA726', EDGE_IN_CATEGORY: '#AB47BC'}
            ...
            net.add_edge(src, dst, color=color, width=width, dashes=dashes, title=title)
    ...
    def visualize_company_view(self, output_path: str='./outputs/knowledge_graph_companies.html') -> str:
        return self.visualize(output_path=output_path, show_features=False)

    def visualize_feature_view(self, feature: str, output_path: str='./outputs/knowledge_graph_feature.html') -> str:
...
def main() -> None:
    ...
    for c in competitors:
        print(f"    → {c['product_name']} ({c['company']}) similarity={c['similarity']:.2f}")
    print('\n── Feature Gaps ─────────────────────────────────')
    ...
    for cs in kg.get_category_stats():
        print(f"  {cs['category']}: {cs['product_count']} products, avg ${cs['avg_price']:.2f}")
    ...
    if top_feature:
        kg.visualize_feature_view(top_feature, f"./outputs/knowledge_graph_feature_{top_feature.replace(' ', '_')}.html")
